...
public class ExcluirSint implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ("confirmar" == e.getActionCommand()) {
			if(opcao == 1) {
			dados.remFisico(dadosSint.getIdxFisico(fisicoLido));
			} else if(opcao == 2) {
			dados.remHumor(dadosSint.getIdxHumor(humorLido));
			} else if(opcao == 3) {
			dados.remLibido(dadosSint.getIdxLibido(libidoLido));
			}else if(opcao == 4) {
			dados.remSecrecao(dadosSint.getIdxSecrecao(secrecaoLido));
			}
			frame.dispose();
		}
